...
test("프롬프트는 지시문·에피소드·기억·이력·원오프·리마인더·질문 순으로 조립한다", async () => {
  ...
  // 긴 질문 → shortQuestionMode 아님 → 날짜 포함, 기본 토큰 1024, 타임아웃 22초
...
test("짧은 질문은 자동으로 short 모드 — 날짜 제외, 토큰 480", async () => {
  ...
  assert.equal(requests[0].body.generationConfig.maxOutputTokens, 480);
});
...
test("shortQuestionMode 명시가 자동 판정보다 우선한다", async () => {
  ...
  assert.equal(forcedLong.requests[0].body.generationConfig.maxOutputTokens, 1024);

  const forcedShort = createHarness();
  await forcedShort.ask("이것은 스물네 글자를 확실히 넘기는 긴 질문 문장입니다", [], { shortQuestionMode: true });
  assert.equal(forcedShort.requests[0].body.generationConfig.maxOutputTokens, 480);
});
...
test("토큰 하한 128, 지정값 우선", async () => {
  ...
  assert.equal(low.requests[0].body.generationConfig.maxOutputTokens, 128);

  const high = createHarness();
  await high.ask("안녕", [], { maxOutputTokens: 2000 });
  assert.equal(high.requests[0].body.generationConfig.maxOutputTokens, 2000);
});
...
test("타임아웃이면 강등 옵션으로 정확히 1회 재시도한다", async () => {
  ...
  assert.equal(requests.length, 2);
  // 재시도: 기억 제외·short 강제·토큰 min(max(128, 900), 256)=256·타임아웃 12초
  assert.ok(promptOf(requests[1]).startsWith("[지시문][원오프]"));
  assert.equal(requests[1].body.generationConfig.maxOutputTokens, 256);
