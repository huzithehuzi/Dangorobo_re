...
- 외형·문구·사운드만 저장할 때 휴식 타이머를 초기화하지 않는다. 휴식 간격이나 기능 활성화가
  바뀌었을 때만 일정을 다시 계산한다.
- 자동 업데이트 확인은 `updateNotifyEnabled`가 켜져 있고 패키징된 실행일 때만 한다. 저장으로
  방금 켰으면 다음 실행을 기다리지 않고 즉시 한 번 확인한다 — 끈 경우는 진행 중인 다운로드를
  중단할 수단이 없어 그대로 두고 다음 실행부터 건너뛴다.
...
- 설정창의 팝오버는 네이티브 `popover`로 띄우고 위치는 CSS anchor로 누른 버튼에 묶는다.
  스크롤 패널 안의 `position: absolute`는 잘리고, 가운데 띄우면 어느 항목을 편집하는 중인지
  알 수 없다. **`position-area`로 잡을 때는 `position-try-order: most-block-size`를 함께 준다** —
  팝오버 높이가 앵커 쪽 남은 공간으로 깎이는데, `position-try-fallbacks`의 뒤집기는 상자가
  넘칠 때만 걸려서(깎이면 안 넘친다) 이 order가 없으면 좁은 쪽에 눌린 채로 남는다.
...
- 날씨 지오코딩의 한국 지명 보정(`weather-service.ts`)은 **직접 조회보다 먼저** 온다. 축약형의
  실패 모드는 "못 찾음"이 아니라 동명의 시골 마을에 성공적으로 걸리는 것이라, 폴백으로 두면
  영영 실행되지 않는다. 보정 목록을 고칠 때는 실제 API로 좌표를 확인한다 — 색인이 언제든
  바뀌고, 틀려도 앱은 조용히 엉뚱한 지역 날씨를 보여준다.
- **잘린 응답(`finishReason: "MAX_TOKENS"`)은 사고를 끈 요청으로 한 번 더 받는다.** 예산 숫자를
  키워 맞추려 하면 프롬프트 길이에 따라 계속 어긋난다.
- **오프너(자동 주기·쓰다듬기·부르기)는 응답 대기 플래그로 겹침을 막는다.** `sessionActive`는
  응답이 온 뒤에야 켜지므로 그것만으로는 대기 중 두 번째 트리거를 막지 못한다.
- **`maxOutputTokens`는 사고 토큰까지 포함한 상한이다.** 호출부가 주는 예산은 "답변 길이"로
  해석하고 사고 몫은 `ask-gemini.ts`가 얹는다(`ASSISTANT_THINKING_HEADROOM_TOKENS`). 경로마다
  예산을 손으로 키우지 말 것 — 작은 예산(오프너 320, 기억 추출 50~300)에서 답변이 문장 중간에
  끊기거나 빈 문자열로 온다.
...
- **잊어달라는 요청은 소프트 삭제만으로 처리하지 않는다.** `insertMemory()`는 같은 키를 다시
  ...
  잊은 사실은 다시 배울 수 없으므로 되돌릴 수단이 없으면 영구 손실이다. 되살리기가 있는
  "기억 관리" 탭은 기본으로 숨어 있으므로, 잊은 기억이 하나라도 있으면 토글과 무관하게
  탭이 나와야 한다(`memory:get-stats`의 `forgottenCount`).
- **잊어달라는 요청은 추출 주기를 기다리지 않고 즉시 처리한다.** 러너는 마지막 질문에서만
  잊기 신호를 찾으므로, 3턴 주기를 기다리면 그 사이 마지막 질문이 바뀌어 요청이 통째로
  사라진다. 판정은 `countTowardExtraction` early return보다 **앞**에 둔다 — 펫 대화 답장은
  카운터를 올리지 않아 그쪽 요청이 영영 버려진다.
...
- 사용자에게 보이는 기능·버전이 바뀌면 README 3종과 `package.json`·`package-lock.json`을 맞춘다.
  **버전을 올릴 때는 `ui/settings/patch-notes.ts`에 그 버전 항목을 3개 언어로 추가한다** —
  설정창 "패치 노트" 탭이 읽는 원본이고, 사용자가 체감하는 변화만 짧게 쓴다(원인·구현·검증은
  `docs/CHANGELOG.md`).
