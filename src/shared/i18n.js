...
(function (root, factory) {
  ...
  const STRINGS = {
    ...
    "weather.todayMorningLabel": { ko: "오늘 오전", en: "This morning", ja: "今日の午前" },
    "weather.todayAfternoonLabel": { ko: "오늘 오후", en: "This afternoon", ja: "今日の午後" },
    "weather.tomorrowMorningLabel": { ko: "내일 오전", en: "Tomorrow morning", ja: "明日の午前" },
    "weather.tomorrowAfternoonLabel": { ko: "내일 오후", en: "Tomorrow afternoon", ja: "明日の午後" },
    "weather.dayLine": { ko: "{{label}} ▲{{max}}° ▼{{min}}°", en: "{{label}} ▲{{max}}° ▼{{min}}°", ja: "{{label}} ▲{{max}}° ▼{{min}}°" },
    ...
    "settings.tab.dev": { ko: "개발자", en: "Developer", ja: "開発者" },

    "settings.dev.subtitle": { ko: "이 탭은 저장되지 않고, 창을 새로 열면 다시 숨겨집니다.", en: "This tab isn't saved — it hides again when you reopen the window.", ja: "このタブは保存されず、ウィンドウを開き直すと再び隠れます。" },
    "settings.dev.expressionHeading": { ko: "표정 강제 전환", en: "Force Expression", ja: "表情を強制切り替え" },
    "settings.dev.expressionNote": { ko: "고른 표정을 다른 상태(알람·쓰다듬기 등)보다 우선해서 계속 보여줍니다.", en: "Keeps the chosen expression showing, overriding other states like alarms or petting.", ja: "選んだ表情を、アラームやなでなでなど他の状態より優先して表示し続けます。" },
    "settings.dev.expression.none": { ko: "강제 안 함", en: "No override", ja: "強制しない" },
    "settings.dev.expression.normal": { ko: "보통", en: "Normal", ja: "通常" },
    "settings.dev.expression.normal_blink": { ko: "눈 감음", en: "Blink", ja: "まばたき" },
    "settings.dev.expression.happy": { ko: "기쁨", en: "Happy", ja: "嬉しい" },
    "settings.dev.expression.angry": { ko: "화남", en: "Angry", ja: "怒り" },
    "settings.dev.expression.sad": { ko: "슬픔", en: "Sad", ja: "悲しい" },
    "settings.dev.expression.alarm": { ko: "알람", en: "Alarm", ja: "アラーム" },
    "settings.dev.expression.shocked": { ko: "놀람", en: "Shocked", ja: "驚き" },
    "settings.dev.alertsHeading": { ko: "알림 즉시 트리거", en: "Trigger Alerts Now", ja: "通知を今すぐ発生" },
    "settings.dev.alertsNote": { ko: "대기 시간을 무시하고 지금 바로 띄웁니다.", en: "Skips the wait and shows it right now.", ja: "待機時間を無視して今すぐ表示します。" },
    "settings.dev.testRestButton": { ko: "휴식 알림 즉시 표시", en: "Show Rest Reminder Now", ja: "休憩通知を今すぐ表示" },
    "settings.dev.testWeatherButton": { ko: "날씨 브리핑 즉시 표시", en: "Show Weather Briefing Now", ja: "天気ブリーフィングを今すぐ表示" },
    "settings.dev.overlayHeading": { ko: "렌더링 디버그 오버레이", en: "Rendering Debug Overlay", ja: "レンダリングデバッグオーバーレイ" },
    "settings.dev.overlayToggle": { ko: "펫 창에 FPS·창 경계 표시", en: "Show FPS and window bounds on the pet window", ja: "ペットウィンドウにFPS・境界を表示" },
    "settings.dev.overlayNote": { ko: "FPS, 창 크기, 클릭 통과 여부를 펫 창 좌상단에 표시합니다.", en: "Shows FPS, window size, and click-through state in the pet window's top-left corner.", ja: "FPS・ウィンドウサイズ・クリック透過状態をペットウィンドウの左上に表示します。" },
    ...
    "settings.general.weatherCityNote": { ko: "알람의 날씨 브리핑과 트레이의 '현재 날씨'에 쓰입니다. 회원가입이 필요 없는 Open-Meteo로 조회하며, 대한민국 지역은 기상청(KMA) 기반 데이터를 우선 사용합니다. 시/군 단위까지만 입력하세요(예: 성남시, 수원시) — 구 단위(예: 강남구)는 찾지 못할 수 있습니다.", en: "Used by alarm weather briefings and the tray's 'Check Weather'. Fetched via Open-Meteo, which needs no sign-up — locations in South Korea prefer Korea Meteorological Administration (KMA) data. Enter down to the city/county level only (e.g. Seongnam, Suwon) — district-level names (e.g. Gangnam-gu) may not be found.", ja: "アラームの天気ブリーフィングとトレイの「現在の天気」に使われます。登録不要のOpen-Meteoで取得し、韓国国内の地域は気象庁(KMA)ベースのデータを優先的に使用します。市・郡レベルまでの入力にしてください(例: 城南市、水原市) — 区レベル(例: 江南区)は見つからない場合があります。" },
    ...
    "settings.customization.presetsNote": { ko: "프리셋은 바디 색상·파츠 스타일·얼굴 무늬/장식/눈/입 설정을 저장합니다. 썸네일을 누르면 적용되지만 폼에만 반영되므로, 유지하려면 저장 버튼을 눌러주세요. 저장·삭제·가져오기는 즉시 반영되며 설정 저장 버튼과 무관합니다. 커스텀 얼굴·바디 이미지는 프리셋마다 따로 저장되고, 내보내기는 이미지까지 함께 담은 ZIP 세트 파일로 나갑니다. 썸네일은 지금 사용 중인 조명·외곽선 설정으로 그려지며, 설정을 저장하거나 이 창을 다시 열면 새로 그려집니다.", en: "Presets store body colors, part styles, and face pattern/cosmetic/eye/mouth settings. Clicking a thumbnail applies it to the form only, so press Save to keep it. Save, delete, and import apply instantly and are independent of the Save button. Custom face and body images are stored per preset, and Export writes a single ZIP set that includes them. Thumbnails are rendered with your current lighting and outline settings, and are redrawn when you save or reopen this window.", ja: "プリセットにはボディカラー・パーツスタイル・顔の模様/装飾/目/口の設定が保存されます。サムネイルを押すと適用されますがフォームにのみ反映されるので、維持するには保存ボタンを押してください。保存・削除・インポートは即座に反映され、設定の保存ボタンとは無関係です。カスタム顔・ボディ画像はプリセットごとに保存され、エクスポートは画像まで含むZIPセットファイルとして出力されます。サムネイルは現在の照明・輪郭線の設定で描画され、設定を保存するかこのウィンドウを再度開くと再描画されます。" },
